...
const NavbarClient: React.FC<Props> = ({ session }) => {
  ...
  return (
    <div className="flex justify-center items-center py-4 w-full bg-zinc-950 ">
      <div className="relative w-[90%] max-w-6xl bg-zinc-900/85 backdrop-blur-md border border-zinc-700/80 rounded-full px-6 py-2.5 flex items-center justify-between">
        ...
        {/* MOBILE DROPDOWN */}
        {open && (
          <div className="absolute top-[calc(100%+12px)] left-0 right-0 bg-zinc-900/97 backdrop-blur-md border border-zinc-700/80 rounded-2xl p-4 flex flex-col gap-1 lg:hidden z-100">
            {["Help", "New", "Recommend"].map((item) => (
              <Link
                key={item}
                href={`/${item.toLowerCase()}`}
